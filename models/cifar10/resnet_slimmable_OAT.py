...
######################## Decoupled Logit Learning based Model with Separate Output Heads

class SlimmableResNet34OAT_SOL(nn.Module):
    def __init__(self, num_classes=10, FiLM_in_channels=1, use2BN=True):
        super(SlimmableResNet34OAT_SOL, self).__init__()
        self.use2BN = use2BN
        ...
        self.conv1 = SlimmableConv2d([3 for _ in width_mult_list], list64, kernel_size=3, stride=1, padding=1, bias=False)
        if self.use2BN:
            self.bn1 = SwitchableDualBN2d(list64)
        else:
            self.bn1 = SwitchableBatchNorm2d(list64)
